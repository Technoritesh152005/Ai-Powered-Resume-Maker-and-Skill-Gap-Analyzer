    ...
    <DashboardLayout>
      <div className="space-y-6 pb-6 sm:space-y-8 sm:pb-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
          <div className="xl:col-span-8 flex min-w-0 flex-col gap-6">
            <SkillProgressGraph
          ...
          <div className="xl:col-span-4 flex min-w-0 flex-col gap-6">
            <div className="flex-1">
              <RecentActivity
        ...
        {dashboardData?.skillGapsSummary && dashboardData.skillGapsSummary.total > 0 && (
          <section className="bg-white/4 backdrop-blur-xl rounded-[32px] sm:rounded-[40px] p-5 sm:p-8 md:p-10 border border-white/8 relative overflow-hidden group shadow-2xl">
            <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-[0.05] transition-opacity">
              <Crown className="w-48 h-48 text-white" />
            </div>

            <div className="relative z-10">
              <h3 className="text-sm font-black text-white uppercase tracking-[0.2em] mb-6 sm:mb-10 flex items-center gap-3 sm:gap-4">
                <div className="w-10 h-10 rounded-2xl bg-danger-600/20 border border-danger-500/30 flex items-center justify-center shadow-lg shadow-danger-900/20 shrink-0">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 md:gap-8">
                <div className="text-center bg-white/5 p-4 sm:p-8 rounded-[24px] sm:rounded-[32px] border border-white/5 group-hover:bg-white/8 transition-all hover:scale-[1.02] duration-300">
                  <div className="text-3xl sm:text-4xl font-black text-danger-400 tracking-tighter mb-2">
                ...
                <div className="text-center bg-white/5 p-4 sm:p-8 rounded-[24px] sm:rounded-[32px] border border-white/5 group-hover:bg-white/8 transition-all hover:scale-[1.02] duration-300">
                  <div className="text-3xl sm:text-4xl font-black text-amber-400 tracking-tighter mb-2">
                ...
                <div className="text-center bg-white/5 p-4 sm:p-8 rounded-[24px] sm:rounded-[32px] border border-white/5 group-hover:bg-white/8 transition-all hover:scale-[1.02] duration-300">
                  <div className="text-3xl sm:text-4xl font-black text-primary-400 tracking-tighter mb-2">
              ...
              <div className="mt-6 sm:mt-10 flex w-full sm:w-fit items-start sm:items-center gap-3 px-4 sm:px-5 py-3.5 bg-success-500/10 border border-success-500/20 rounded-2xl group/btn cursor-default">
                <ShieldCheck className="w-5 h-5 text-success-400 transition-transform group-hover/btn:scale-110 shrink-0 mt-0.5 sm:mt-0" />
...
const UserProgressPanel = ({ userProgress, roadmap, loading }) => {
  ...
  return (
    <section className="bg-white/4 backdrop-blur-xl rounded-[28px] sm:rounded-[32px] p-5 sm:p-7 border border-white/8 shadow-2xl">
...
const ProgressMetric = ({ label, value, accent }) => {
  const isLongValue = typeof value === 'string' && value.length > 16;

  return (
  <div className="rounded-[24px] border border-white/8 bg-white/5 p-5 min-w-0">
    <p className="text-[10px] font-black uppercase tracking-widest text-neutral-500">{label}</p>
    <p className={`mt-3 break-words font-black tracking-tight ${isLongValue ? 'text-lg sm:text-2xl' : 'text-3xl'} ${accent}`}>{value}</p>
  </div>
  );
};
